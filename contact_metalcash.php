...
$(document).ready(function() {
	$('a[rel*=modalbox]').leanModal({ top : 50, closeButton: ".modal_close" });	
});
</script>
...
if ($_POST['contact']) {
	...
	/* Verifier nom et email uniquement si non authentifié */
	if (empty($_POST['name'])) {
		$errorname1 = $ERRORTAG;
		$errorname2 = $ERRORMSG;
		$error = 1;
	...
	/* Verification Téléphone si paramètré */
	if ($_POST['phone'] != "") {
...
<?php /**/ ?>
		...
		<tr><td><?=FIELD_NAME?>:</td><td ><?=$errorname1?><input onclick="this.style.color='#000';" type="text" name="name" value="<?=$_POST['name']?>" style="width: 220px;"><?=$errorname2?></td></tr>
		<tr><td><?=FIELD_EMAIL?>:</td><td ><?=$erroremail1?><input onclick="this.style.color='#000';" type="text" name="email" value="<?=$_POST['email']?>" style="width: 220px;"><?=$erroremail2?></td></tr>
		<? } else { ?>
		...
		<? } ?>
		<tr><td><?=FIELD_PHONE?>:</td><td ><?=$errorphone1?><input onclick="this.style.color='#000';" type="text" name="phone" value="<?=$_POST['phone']?>" style="width: 220px;"><?=$errorphone2?></td></tr>
		<tr><td><?=FIELD_OBJECT?>:</td><td ><select name="subject" style="width: 225px;"><? foreach ($contact_subject as $v) { if ($_POST['subject'] == $v) $selected = " selected"; else unset($selected); ?><option value="<?=$v?>"<?=$selected?>><?=$v?></option><? } ?></select></td></tr>
		<tr><td colspan="2"><?=FIELD_MSG?>:<br><?=$errorquestion1?><textarea name="question" cols="55" rows="11"><?=$_POST['question']?></textarea><?=$errorquestion2?></td></tr>
		<tr><td colspan="2" align="center"><?=$errorcapcha1?><div class="g-recaptcha" data-sitekey="6Lf7n5MiAAAAAP7_TlZ0Uw2-5onMv5NbnUUSxkkX"></div><?=$errorcapcha2?></td></tr>
		</table><p>
		...
		<p>&nbsp;</p>
		<h2><?=FAQ?></h2>
		<img src="/imgs/icons/arrow.png" alt="->" align="absmiddle" border="0" width="20" height="20"> <a href="<?=LINK_SELL?>" style="font-size: 16px;"><?=CLICK_HOW_TO_SELL?></a><p>
		<img src="/imgs/icons/arrow.png" alt="->" align="absmiddle" border="0" width="20" height="20"> <a href="<?=LINK_FAQ?>" style="font-size: 16px;"><?=CLICK_FAQ?></a><p>
